...
class PIDJointVelocityController(Controller):
    ...
    def step_control(self, target_position, target_velocity, target_acceleration):
        ...
        # Kw is used to make the integration easier for us, if we set it to 0, we wouldn't control accumulated errors.


        controller_velocity = target_velocity + self.Kp @ error + self.Kd @ (target_velocity - current_velocity) + self.Ki @ self.integ_error
